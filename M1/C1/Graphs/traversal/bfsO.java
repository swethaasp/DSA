...
//https://www.youtube.com/watch?v=-tgVpUgsQ5k&list=PLgUwDviBIf0rGEWe64KWas0Nryn7SCRWw&index=4
  ...
  class Solution {
    ...
    public ArrayList<Integer> bfsOfGraph(int V, ArrayList<ArrayList<Integer>> adj) {
        ...
        while(!q.isEmpty()){
            Integer node=q.poll(); //take node at down in queue
            bfs.add(node);  //add that node to bfs
            
            for(Integer it : adj.get(node)){  //0->1 2 3 
              if(vis[it]==0){
                  vis[it]=1;
                  q.add(it);
              } 
